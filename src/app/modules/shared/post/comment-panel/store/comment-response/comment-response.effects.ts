...
import * as PostCommentsActions from '../post-comment/post-comment.actions';
...
export default class CommentResponseEffects {
  @Effect() getCommentsByPostId = this.actions$.pipe(
    ...
    switchMap((action: CommentResponseActions.GetCommentResponses) => {
      return this.commentResponseService.getAllByCommentId(action.payload.commentId)
        .pipe(map((res: CommentResponseModel[]) => {
          return new CommentResponseActions.SetCommentResponses({[action.payload.commentId]: res});
        }));
    })
  );
  ...
  @Effect() removeCommentById = this.actions$.pipe(
    ...
    switchMap((action: CommentResponseActions.RemoveById) => {
      return this.commentResponseService.delete(action.payload.id)
        .pipe(map((res: CommentResponseModel) => {
  ...
  @Effect() create = this.actions$.pipe(
    ...
    switchMap((action: CommentResponseActions.Create) => {
      return this.commentResponseService.create(action.payload)
        .pipe(map((res: CommentResponseModel) => {
  ...
  @Effect() unlike = this.actions$.pipe(
    ...
    tap((action: CommentResponseActions.Unlike)=>
      this.store$.dispatch(new CommentResponseActions.UnlikeInState({parentCommentId: action.payload.parentCommentId,
        id: action.payload.id}))
    ),
