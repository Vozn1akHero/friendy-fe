...
export class DialogMessagesEffects {
  @Effect() getInitialDialog = this.actions$.pipe(
    ...
    switchMap((action: DialogActions.GetInitialDialog) => {
      return this.dialogService
        .getMessagesInDialog(action.payload.to, 1, 20)
        .pipe(
          map(response => {
            return new DialogActions.SetDialog(response.reverse());
          })
        );
    })
  );
  ...
  @Effect() getMessagesInDialog = this.actions$.pipe(
    ...
    switchMap((action: DialogActions.GetDialog) => {
      return this.dialogService
        .getMessagesInDialog(action.payload.to, action.payload.page, 20)
        .pipe(
          map(response => {
            return new DialogActions.FillDialog(response.reverse());
          })
        );
    })
  );
