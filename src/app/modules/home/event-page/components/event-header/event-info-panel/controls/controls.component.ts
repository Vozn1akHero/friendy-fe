...
export class ControlsComponent implements OnInit {
  ...
  nonParticipantStatus: PARTICIPATION_STATUS = PARTICIPATION_STATUS.NonParticipant;
  ...
  getUserParticipationStatus(){
    this.store.dispatch(new ParticipationStatusActions.Get({eventId: this.eventId}));
    this.participationStatusLoaded$ = this.store.select(e=>e.eventPageParticipation.loaded);
    this.participationStatus$ = this.store.select(e=>e.eventPageParticipation.participationStatus);
  }

  leave(){
    this.store.dispatch(new ParticipationStatusActions.Leave({eventId: this.eventId}))
  }

  removeParticipationRequest() {
    this.store.dispatch(new ParticipationStatusActions.RemoveRequest({eventId: this.eventId}))
  }

  sendParticipationRequest() {
    this.store.dispatch(new ParticipationStatusActions.SendRequest({eventId: this.eventId}))
  }
}
